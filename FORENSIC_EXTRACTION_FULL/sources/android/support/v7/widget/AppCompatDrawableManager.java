...
public final class AppCompatDrawableManager {
    ...
    private Drawable loadDrawableFromDelegates(@NonNull Context context, @DrawableRes int i) {
        ...
        if (this.mDelegates != null && !this.mDelegates.isEmpty()) {
            ...
            long jCreateCacheKey = createCacheKey(typedValue);
            cachedDrawable = getCachedDrawable(context, jCreateCacheKey);
            if (cachedDrawable == null) {
                if (typedValue.string == null || !typedValue.string.toString().endsWith(".xml")) {
                    drawableCreateFromXmlInner = cachedDrawable;
                } else {
                    try {
                        ...
                        drawableCreateFromXmlInner = inflateDelegate != null ? inflateDelegate.createFromXmlInner(context, xml, attributeSetAsAttributeSet, context.getTheme()) : cachedDrawable;
                        if (drawableCreateFromXmlInner != null) {
                            try {
                                drawableCreateFromXmlInner.setChangingConfigurations(typedValue.changingConfigurations);
                                if (addDrawableToCache(context, jCreateCacheKey, drawableCreateFromXmlInner)) {
                                    cachedDrawable = drawableCreateFromXmlInner;
                                }
                            } catch (Exception e) {
                                e = e;
